...
public class GoogleCalendarSyncService {
    private static final String DEFAULT_GOOGLE_CLIENT_ID = "327994023632-cut8g0d28j0b0sa7fuv92opgvi7s7dtr.apps.googleusercontent.com";
    private static final String DEFAULT_GOOGLE_REDIRECT_URI = "http://localhost:8765/oauth2callback";
    ...
    private record OAuthAuthorization(String code, String codeVerifier, String redirectUriUsed) {}
    ...
    public SyncResult syncFromGoogleCalendar(User user) throws Exception {
        ...
        String redirectUri = resolveGoogleRedirectUri();
        if (clientId.isBlank() || redirectUri.isBlank()) {
            return new SyncResult(0, 0, buildMissingConfigMessage(clientId, clientSecret, redirectUri));
        }

        OAuthAuthorization authorization = requestAuthorizationCode(clientId, redirectUri);
        if (authorization == null || authorization.code() == null || authorization.code().isBlank()) {
            return new SyncResult(0, 0, "Google authorization was cancelled.");
        }

        String accessToken = exchangeAuthorizationCodeForAccessToken(
                authorization.code(),
                authorization.codeVerifier(),
                clientId,
                clientSecret,
                authorization.redirectUriUsed()
        );
    ...
    private OAuthAuthorization requestAuthorizationCode(String clientId, String redirectUri) throws Exception {
        ...
        boolean localhostRedirect = "localhost".equalsIgnoreCase(redirect.getHost()) || "127.0.0.1".equals(redirect.getHost());
        int port = localhostRedirect ? 0 : (redirect.getPort() == -1 ? 80 : redirect.getPort());
        String callbackPath = (redirect.getPath() == null || redirect.getPath().isBlank()) ? "/" : redirect.getPath();
        ...
        int effectivePort = callbackServer.getAddress().getPort();
        String effectiveRedirectUri = localhostRedirect
                ? redirect.getScheme() + "://" + redirect.getHost() + ":" + effectivePort + callbackPath
                : redirectUri;
        ...
        try {
            ...
            return new OAuthAuthorization(code, codeVerifier, effectiveRedirectUri);
        } finally {
            callbackServer.stop(0);
        }
    }
    ...
    private String resolveGoogleClientSecret() {
        return firstNonBlank(
    ...
    private String resolveGoogleRedirectUri() {
        return firstNonBlank(
                ...
                System.getProperty("google.redirect.uri"),
                DEFAULT_GOOGLE_REDIRECT_URI
        );
    }

    private String buildMissingConfigMessage(String clientId, String clientSecret, String redirectUri) {
        List<String> missing = new ArrayList<>();
        if (clientId == null || clientId.isBlank()) missing.add("GOOGLE_CLIENT_ID");
        if (redirectUri == null || redirectUri.isBlank()) missing.add("GOOGLE_REDIRECT_URI");
        return "Google OAuth configuration is missing: " + String.join(", ", missing) + ".";
    }
