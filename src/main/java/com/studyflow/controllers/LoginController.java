...
public class LoginController implements Initializable {
    ...
    private void handleLogin() {
        ...
        try {
            // ── Route: admin → AdminLayout, user → MainLayout ──
            if (isAdmin(user)) {
            ...
            showError(e.getMessage()); // shows exact missing path
            e.printStackTrace();
            resetButton();
        }
    }
     ...
     * Symfony stores roles as JSON array e.g. ["ROLE_ADMIN","ROLE_USER"]
     */
    private boolean isAdmin(User user) {
        // You can also check by email for a quick test:
        // return "admin@rlife.com".equals(user.getEmail());
        return user.getRoles() != null && user.getRoles().contains("ROLE_ADMIN");
    }

    @FXML private void forgotPassword() { showError("Password reset is not yet implemented."); }

    @FXML private void goToRegister() throws IOException { App.setRoot("views/Register"); }

    @FXML private void goToLanding() throws IOException { App.setRoot("views/Landing"); }
